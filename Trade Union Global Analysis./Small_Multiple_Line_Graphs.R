...
custom_colors <- setNames(
  c("steelblue", "darkorange", "red", "limegreen", "deepskyblue", "gold", "purple", "#FF00FF", "pink"),
...
# Adjusted common theme for white background
common_theme <- function() {
  theme(
    plot.background = element_rect(fill = "white", color = NA),
    panel.background = element_rect(fill = "white", color = NA),
    panel.grid.major = element_line(color = "grey90"), # Light grey grid lines
    panel.grid.minor = element_line(color = "grey95"), # Even lighter for minor grid lines
    text = element_text(color = "black"), # Black text for better visibility
...
# ... rest of your code for plotting (p1 and p2) ...

# Apply common_theme to the plots as before
p1 + common_theme()
p2 + common_theme()
